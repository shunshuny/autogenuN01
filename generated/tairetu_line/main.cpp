...
int main() {
  ...
  const double t0 = 0;
  cgmres::Vector<27> x0;
  x0 << 0, 0, 0.7, 0, -2.0, 0.7, 0, -4.0, 0.7, 0, 0, 0.7, 0, -2.0, 0.7, 0, -4.0, 0.7, 0, 0, 0.7, 0, -2.0, 0.7, 0, -4.0, 0.7;

  // Initialize the solution of the C/GMRES method.
  constexpr int kmax_init = 5;
  cgmres::ZeroHorizonOCPSolver<cgmres::OCP_tairetu_line, kmax_init> initializer(ocp, settings);
  cgmres::Vector<6> uc0;
  uc0 << 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001;
  ...
  cgmres::SingleShootingCGMRESSolver<cgmres::OCP_tairetu_line, N, kmax> mpc(ocp, horizon, settings);
  ...
  const std::string log_name("../log/tairetu_line"); 
